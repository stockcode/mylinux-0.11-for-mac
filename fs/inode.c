...
void invalidate_inodes(int dev)
{
    ...
    for(i=0 ; i<NR_INODE ; i++,inode++) {   // 扫描 i 节点表指针数组中的所有 i 节点。
        wait_on_inode(inode);               // 等待该 i 节点可用(解锁)。
        if (inode->i_dev == dev) {          // 如果是指定设备的 i 节点
...
{
    struct m_inode * inode;
    static struct m_inode * last_inode = inode_table; //last_inode指向i节点表第一项。
    int i;
    ...
    } while (inode->i_count); // 如果 i 节点又被其他占用的话，则重新寻找空闲 i 节点。
...
struct m_inode * iget(int dev,int nr)
{
    ...
    empty = get_empty_inode();  //从 inode_table[32]中申请一个空闲的i 节点

    //扫描 i 节点表。寻找指定节点号的 i 节点，并递增该节点的引用次数
    inode = inode_table;
    while (inode < NR_INODE+inode_table) {
        //如果当前扫描的节点的设备号不等于指定的设备号或节点号不等于指定的节点号，则继续扫描
        ...
        wait_on_inode(inode);
        //在等待该节点解锁的阶段，节点表可能会发生变化，所以再次判断，如果发生了变化，则再次重新扫描整个 i 节点表
        if (inode->i_dev != dev || inode->i_num != nr) {
            inode = inode_table;
            continue;
        }
        ...
        if (inode->i_mount) {
            ...
            //如果该 i 节点是其他文件系统的安装点，则在超级块表中搜寻安装此 i 节点的 超级块。
            ...
            //将此 i 节点写盘。从安装在此 i 节点文件系统的超级块上取设备号，并令 i 节点号为1，
        ...
        //已经找到对应的 i 节点，因此放弃临时申请的空闲节点，返回该找到的 i 节点
        if (empty)
            iput(empty);
        return inode;
    }
    //如果在 i 节点表中没有找到指定的 i 节点，则利用前面申请的空闲 i 节点在 i 节点表中建立该节点。
    //并从相关设备上读取该 i 节点信息，返回该 i 节点
    if (!empty)
        return (NULL);
    inode=empty;
    inode->i_dev = dev;
...
static void read_inode(struct m_inode * inode)
{
    ...
    //该 i 节点的逻辑块号=（启动块+超级块）+i 节点位图占用的块数+逻辑块位图占用的块数+
    // （i 节点号-1）/每块含有的i节点数
    block = 2 + sb->s_imap_blocks + sb->s_zmap_blocks +
            (inode->i_num-1)/INODES_PER_BLOCK;
    //从设备是读取该 i 节点所在的逻辑块，并将 inode 指针指向对应i 节点信息
