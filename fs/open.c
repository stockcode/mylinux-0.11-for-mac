...
int sys_open(const char * filename,int flag,int mode)
{
    ...
    printk("sys_open->filename:%s\n", get_from_fs(filename));
    //将用户设置的模式与进程的模式屏蔽码相与，产生许可的文件模式
    mode &= 0777 & ~current->umask;
    for(fd=0 ; fd<NR_OPEN ; fd++)      //遍历当前进程的 filp
        if (!current->filp[fd])         //直到获取一个空闲项，fd就是这个空闲项的项号
            break;
    if (fd>=NR_OPEN)    //如果此条件成立，说明 filp[20]中没有空闲项了，直接返回
        return -EINVAL;
    //设置执行时关闭文件句柄位图，复位对应比特位
    current->close_on_exec &= ~(1<<fd);
    f=0+file_table;                 //获取 file_table[64]首地址
    for (i=0 ; i<NR_FILE ; i++,f++) //遍历 file_table[64]
        if (!f->f_count) break;     //直到获取一个空闲项，f就是这个空闲项的指针
    if (i>=NR_FILE)                 //如果此条件成立，说明 file_table[64]中没有空闲项了，直接返回
        return -EINVAL;
    //将进程的 filp[20]与 file_table[64]挂接，并增加引用计数
