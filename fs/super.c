...
struct super_block * get_super(int dev)
{
    ...
    while (s < NR_SUPER+super_block)
        ...
        // 如果是则返回该超级块的指针。否则就重新对超级块数组再搜索一遍，因此 s 又指向超级块数组
        // 开始处。
        if (s->s_dev == dev) {
            wait_on_super(s);
            if (s->s_dev == dev)
...
void put_super(int dev)
{
    ...
    lock_super(sb);
    sb->s_dev = 0;
    // 然后释放该设备 i节点位图和逻辑块位图在缓冲区中所占用的缓冲块。
...
static struct super_block * read_super(int dev)
{
    ...
    *((struct d_super_block *) s) =
            *((struct d_super_block *) bh->b_data); //将缓冲区中的超级块复制到 super_block[0]
    brelse(bh);                     //释放缓冲块
    ...
    for (i=0;i<I_MAP_SLOTS;i++)     //初始化 s_imap[8]、s_zmap[8]
        s->s_imap[i] = NULL;
    for (i=0;i<Z_MAP_SLOTS;i++)
        s->s_zmap[i] = NULL;

    // 然后从设备上读取 i 节点位图和逻辑块位图信息，并存放在超级块对应字段中。
    block=2;                        //根设备的第一块是超级块，第二块开始是 i 节点位图和逻辑块位图

    //把根设备上 i 节点位图所占用的所有逻辑块读到缓冲区，分别挂在到 s_imap[8] 中
    ...
    //把根设备上逻辑块位图所占用的所有逻辑块读到缓冲区，分别挂在到 s_zmap[8] 中
    ...
    s->s_imap[0]->b_data[0] |= 1;   //牺牲掉一个 i 节点，以防止查找算法返回0
    s->s_zmap[0]->b_data[0] |= 1;   //把位图的第一个字节的最低位设置为 1，表示该位图的第一个块已被使用
...
void mount_root(void)
{
    ...
    for(i=0;i<NR_FILE;i++)              //初始化 file_table[64]，为后续程序做准备
        file_table[i].f_count=0;

    // 如果根文件系统所在设备是软盘的话，就提示“插入根文件系统盘，并按回车键”，并等待按键。
    if (MAJOR(ROOT_DEV) == 2) {         //2代表软盘
