 ...
 * 缓冲区以及目录项本身(作为一个参数 - res_dir)。并不读目录项的 i 节点
 * - 如果需要的话需自己操作。
 *
   ...
   directory-inode. NOTE! We set mounted, so that we can iput the new dir */
    ...
    while (i < entries) {
        ...
        if ((char *)de >= BLOCK_SIZE+bh->b_data) {
            ...
            // 在读入下一目录项数据块。若这块为空，则只要还没有搜索完目录中的所有目录项，
            // 就跳过该块， 继续读下一目录项数据块。若该块不空，就让 de 指向该目录项数据块，继续搜索。
...
//如果没有定义 NO_TRUNCATE，则若文件名长度超过最大长度 NAME_LEN，则截短之。
    ...
    while (1) {
        ...
        // 更新该目录的长度值(加上一个目录项的长度，设置目录的 i 节点已修改标志，再更新该目录的改变时间为
        // 当前时间
...
static struct m_inode * get_dir(const char * pathname)
{
    ...
    while (1) {
        ...
        inr = de->inode;        //通过目录项找到 i 节点号
        idev = inode->i_dev;    //注意，这个 inode是根 i 节点，这里通过根 i 节点找到设备号
        brelse(bh);
        iput(inode);
 ...
 * 自己的相应函数，但对于像修改模式'chmod'等这样的命令，该函数已足够用了。
 */
               ...
               struct m_inode ** res_inode)
{
    ...
    printk("open_namei->pathname:%s\n", get_from_fs(pathname));
